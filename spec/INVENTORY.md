...
| 14 | Casper | **Finality requires > 2/3** bonded stake; fringe = one message per bonded validator (antichain) | `sdk/.../consensus/Stake.scala:8`, `block-storage/.../dag/Finalizer.scala:76,133` | opaque `sdk::consensus::FinalityCertificate` capability (exact `3·stake > 2·total`) + `BTreeMap<S, NonNegI64>` bonds | `Rchain/Casper/{Stake,Fringe}.lean` | **threshold + quorum intersection proven; fringe antichain stated** |
