...
`Rchain.Sort`'s 69 comparator-law axioms, which `Ty.lean` does **not** import).
...
- `Ty.lean` imports only `Par`/`Cmp`/`Rho` (not `Sort`), so it is independent of the 69 residual
...
- **No discharge of the 69 residual `Sort.lean` axioms** — the type system is independent of Law 1's
  total-order bundle.
