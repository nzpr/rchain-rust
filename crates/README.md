...
| `node` | `node/` | **in progress** (configuration: CLI + HOCON merge + `NodeConf`; diagnostics: metric registry + Prometheus/InfluxDB encoders; api: `RhoExpr` Par⇄JSON tree; web: transaction DTOs; effects/runtime: REPL/console); gRPC/runtime-wiring pending |
