...
| `node` | `node/` | **in progress** (configuration: CLI + HOCON merge + `NodeConf`; diagnostics: metric registry + Prometheus/InfluxDB encoders; api: `RhoExpr` Par⇄JSON tree + WebApi DTOs + conversion fns; web: transaction DTOs; effects/runtime: REPL/console; dag: block-requester/DAG-manager stubs); gRPC/runtime-wiring pending |
