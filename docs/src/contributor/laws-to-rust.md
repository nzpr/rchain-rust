...
| **14** | Finality requires **strictly > 2/3** of bonded stake; the fringe is one message per bonded validator (an antichain). | `sdk/src/consensus.rs` — opaque `FinalityCertificate` capability and exact `is_super_majority`; the DAG estimator can advance only after constructing that capability against one committee snapshot | `cargo test -p rchain-sdk consensus` (including property tests) + `cargo test -p rchain-block-storage finalizer`; quorum intersection is proved in `Rchain/Casper/Stake.lean` |
